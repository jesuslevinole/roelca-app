...
export const FormularioOperacion = ({ estado, initialData, onClose, onMinimize, onRestore, catalogosCacheados }: FormProps) => {
  ...
  useEffect(() => {
    ...
    if (maestrosDelCliente.length > 0) {
      ...
      const mapped = detalles.map((d:any) => {
        ...
        const maestroAsociado = maestrosDelCliente.find((m:any) => m.id === d.convenioId || m.id === d.convenio || m.numeroConvenio === d.convenio || m['# de Convenio'] === d.convenio);

        return {
          id: d.id,
          tarifaBaseId: tarifaId,
          descripcion: nombreVisible,
          // Guardamos datos financieros para autollenar la pestaña "Por Cobrar"
  ...
  useEffect(() => {
    const resolverVariablesDeFlujo = async () => {
      ...
      try {
        ...
        // ✅ AUTO-LLENAMOS LA PESTAÑA DE POR COBRAR
        setFormData(prev => ({ 
            ...prev, 
  ...
  useEffect(() => {
    ...
    const utilidad = conv - formData.conversionProv; // Ingreso - Gasto
