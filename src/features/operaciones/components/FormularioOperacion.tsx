...
export const FormularioOperacion = ({ estado, initialData, onClose, onMinimize, onRestore, catalogosCacheados }: FormProps) => {
  ...
  useEffect(() => {
    ...
    const fLatina = `${d}/${m}/${y}`; const fUS = `${m}/${d}/${y}`; const fISO = `${y}-${m}-${d}`; 

    let tcEncontrado = null;
    for (const tc of catalogoTC) {
      const vals = Object.values(tc).map(v => String(v).trim());
      if (vals.includes(fLatina) || vals.includes(fUS) || vals.includes(fISO)) {
        const keys = Object.keys(tc);
        const valKey = keys.find(k => k.toLowerCase().includes('dof') || k.toLowerCase().includes('valor') || k.toLowerCase() === 'tc' || k.toLowerCase().includes('cambio'));
        if (valKey) tcEncontrado = Number(String(tc[valKey]).replace(/[^0-9.-]+/g, ""));
        else {
          const posiblesRates = vals.map(v => parseFloat(v.replace(/[^0-9.-]+/g, ""))).filter(n => !isNaN(n) && n > 15 && n < 25);
    ...
    if (maestros.length > 0) {
      ...
      const mapped = detalles.map((d:any) => {
        ...
        return {
          ...
          monedaMaestro: d.moneda || maestros[0].moneda || ID_USD,
  ...
  useEffect(() => {
    const resolverFlujo = async () => {
      ...
      try {
        ...
        // Auto-llenamos los valores base del cliente para la pestaña "Por Cobrar"
  ...
  useEffect(() => {
    const fact = formData.facturadoEnCobrar; const mon = formData.monedaConvenioCliente; 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const operacionData: Omit<Operacion, 'ref'> = { ...datosLimpios, convenioNombre: detalleDoc?.descripcion || 'Sin descripción', status: statusCalculado, tienePdfDoda: !!pdfDoda, cantPdfsEntrys: pdfsEntrys.filter(Boolean).length };
      if (initialData) alert(`Operación actualizada correctamente.`); else { await guardarOperacionSegura(operacionData); alert('Operación guardada exitosamente'); }
      onClose();
