...
export const FormularioConvenioProveedor = ({ estado, initialData, registrosExistentes, onClose, onMinimize, onRestore }: FormProps) => {
  ...
  useEffect(() => {
    const savedConfig = localStorage.getItem('formConfig_convenioProveedor');
    if (savedConfig) {
      setRequiredFields(JSON.parse(savedConfig));
    } else {
      setRequiredFields(['proveedorId', 'fechaConvenio', 'fechaVencimiento', 'monedaId', 'credito']);
    }
  }, []);
  ...
  const toggleRequired = (fieldName: string) => {
    ...
    localStorage.setItem('formConfig_convenioProveedor', JSON.stringify(newRequired));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (isRequired('proveedorId') && !formData.proveedorId) {
          ...
          <div style={{ display: estado === 'minimizado' ? 'none' : 'block', padding: '10px 0', maxHeight: '75vh', overflowY: 'auto', overflowX: 'hidden' }}>
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-grid" style={{ gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                ...
                <div className="form-group">
                  <label className="form-label">Proveedor {isRequired('proveedorId') && <span style={{ color: '#ff4d4d' }}>*</span>}</label>
                  <SearchableSelect 
                    ...
                    required={isRequired('proveedorId')}
                  />
                </div>
