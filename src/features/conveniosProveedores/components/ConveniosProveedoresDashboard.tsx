...
export const ConveniosProveedoresDashboard: React.FC = () => {
  ...
  const [registros, setRegistros] = useState<ConvenioProveedorRecord[]>([]);
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, 'convenios_proveedores'), (snapshot) => {
      ...
      // Ordenar por número de convenio descendente
      data.sort((a, b) => {
        const numA = parseInt(a.numeroConvenio.replace('CPRV-', ''), 10) || 0;
        const numB = parseInt(b.numeroConvenio.replace('CPRV-', ''), 10) || 0;
        return numB - numA;
      });
    ...
    <>
      ...
      <div className="module-header" style={{ justifyContent: 'space-between', paddingBottom: '16px' }}>
        <h1 className="module-title" style={{ fontSize: '1.2rem', color: '#8b949e' }}>
          Proveedores &gt; <span style={{ color: '#f0f6fc', fontWeight: 'bold' }}>Convenio de Proveedores</span>
        </h1>
